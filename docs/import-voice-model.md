...
本工具的本地 TTS 引擎走 **GPT-SoVITS V4** 路线（48kHz 输出、修了 V3 金属音）。社区分享的 V4 模型通常是一组裸文件（`*.ckpt + *.pth + 参考音频.wav`），需要补一个 `meta.json` + 重打包成 zip 才能被本工具识别。本文讲清楚怎么打包。

> ⚠️ V2/V3 模型当前**不兼容**（serve.py 在 meta.json 声明 v2/v3 时直接拒绝）。
> 必须用 V4 模型；社区里 V2 模型偶有但 V4 是主流（2025-04 起所有更新都基于 V4）。
...
| 来源 | 内容 | 备注 |
|---|---|---|
| [aihobbyist ModelScope（中文 V4）](https://www.modelscope.cn/models/aihobbyist/GPT-SoVITS_Model_Collection/files) | 中文原神/星铁角色 V4 | **首选**，目录路径如 `原神/中文/<角色>` |
| [AI-Hobbyist/GPT-SoVits-V2-models HF（EN/JA）](https://huggingface.co/AI-Hobbyist/GPT-SoVits-V2-models) | 仓库名 V2 但内部已升级，含 V4；只有 `Genshin_Impact/EN/` 和 `Genshin_Impact/JA/`（**无中文**） | 中文走 ModelScope |
...
| `gpt_sovits_version` | 否 | 默认 `v4`；runtime 当前**仅支持 v4**，meta.json 声明 v2/v3 会被拒。仅当确认模型是 V4 训练时才填 v4 |
...
### Q：导入后试听报"加载模型权重失败"
- 检查 `.ckpt` / `.pth` 文件是不是 V1/V2/V3 版本（与 V4 runtime 不兼容，meta.json 会被守门拒绝）
...
从 ModelScope 下载（V4 中文）：`https://www.modelscope.cn/models/aihobbyist/GPT-SoVITS_Model_Collection/files`，进 `原神/中文/纳西妲/`，得到（示意）：
...
| gpt_sovits_version | string | "v4" | GPT-SoVITS 版本（runtime 仅支持 v4；填 v2/v3 会被拒绝） |
